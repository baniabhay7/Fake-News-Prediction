...
When changing the live prediction behavior, edit `app.py` and `retrain_models.py` together. `src/model.py` is **not** wired in — changes there will not affect predictions until you also rewrite `app.py`'s loader.
...
- `data/training_data.csv` is gitignored and not committed; `retrain_models.py` will fail or fall back unless you supply it (or a missing `download_dataset.py`).
...
- `config.py` constants (`MAX_FEATURES=10000`, `MIN_DF=2`, `MAX_DF=0.8`) are **not** what `retrain_models.py` actually uses (it hardcodes `max_features=5000` with default `min_df`/`max_df`). Changing `config.py` will not change retraining behavior.

### Models directory

`models/*.joblib` artifacts are committed to the repo (`.gitignore` has the `models/*.joblib` line commented out intentionally). The expected filenames are exactly: `vectorizer.joblib`, `naive_bayes_model.joblib`, `logistic_regression_model.joblib`, `random_forest_model.joblib`, `svm_model.joblib`. `retrain_models.py` derives these from `model_name.lower().replace(' ', '_') + '_model.joblib'` — renaming a model in the dict will silently produce a file the app can't find.
